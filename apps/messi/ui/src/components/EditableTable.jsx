...
const EditableTable = ({ 
  ...
  return (
    <div className="editableTable-container">
      <div className="editableTable-table-controls">
        {showAddRow && (
          <button onClick={handleAddRow} className="editableTable-add-row-btn">
            {addRowText}
          </button>
        )}
        {showPasteButton && (
          <button onClick={handlePasteClick} className="editableTable-paste-btn">
            클립보드에서 붙여넣기 (탭으로 분리)
          </button>
        )}
        {showCopyButton && (
          <button onClick={handleCopyTSV} className="editableTable-copy-btn">
            클립보드에 복사 (탭으로 분리)
          </button>
        )}
        {onUpdate  && (
          <button 
            ...
            className="editableTable-update-selected-btn"
        ...
        {onDelete && selectedRows.length > 0 && (
          <button 
            ...
            className="editableTable-delete-selected-btn"
        ...
        {onAction && selectedRows.length > 0 && (
          <button 
            ...
            className="editableTable-action-selected-btn"
            disabled={selectedRows.length === 0}
          >
      ...
      <table className="editableTable-table">
        <thead>
          <tr>
            {onDelete && (
              <th className="editableTable-select-column">
                <input
                  type="checkbox"
        ...
        <tbody>
          {getSortedData().map((row, rowIndex) => (
            <tr key={rowIndex} className={selectedRows.includes(rowIndex) ? 'selected-row' : ''}>
              {onDelete && (
                <td className="editableTable-select-column">
                  <input
                    type="checkbox"
                ...
                <td key={col.key}>
                  {onUpdate && editingCell?.rowIndex === rowIndex && editingCell?.columnKey === col.key ? (
                    <input
                      ...
                      autoFocus
                      className="editableTable-cell-input"
                    />
                  ) : (
                    <div
                      className="editableTable-cell-content"
              ...
              <td>
                <button
                  ...
                  className="editableTable-delete-btn"
                  title={deleteRowText}
                >
                  {deleteRowText}
                </button>
